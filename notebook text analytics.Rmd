...
```{r change the text and make sentiment words, include=FALSE}
reviews_df$text_full <- as.character(reviews_df$text_full)  %>% 
  tolower() %>% 
  {gsub(":( |-|o)*\\("," SADSMILE ", .)} %>%       # Find :( or :-( or : ( or :o(
  {gsub(":( |-|o)*\\)"," HAPPYSMILE ", .)} %>%     # Find :) or :-) or : ) or :o)
  {gsub("(\"| |\\$)-+\\.-+"," NUMBER", .)} %>%     # Find numbers
...
all_words <- reviews_df[,] %>%
             unnest_tokens("text_full", output = "word") %>%
...
```{r sentiment score}
pol <- polarity(reviews_df[, "text_full"])$all
reviews_df$polarity <- pol[, "polarity"]
reviews_df$sent_bing  <- get_sentiment(reviews_df$text_full, method = "bing")

```


```{r}
pos_reviews      <- reviews_df %>% filter(happy == "happy")
neg_reviews      <- reviews_df %>% filter(happy == "unhappy")
...
Histograms of sentiment split on Happy/Unhappy on polarity
```{r}
ggplot( ,aes(polarity)) +
    geom_histogram(aes(fill = "Happy"),   data = pos_reviews, alpha = 0.5) +
    geom_histogram(aes(fill = "Unhappy"), data = neg_reviews, alpha = 0.5) +
